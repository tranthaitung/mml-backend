...
const PORT = 8000;
const API_KEY = "dt775crxd3mq";
const API_SECRET =
  "vcdqkscu4ekchk3q433978z3xmab9887yu6kz4z5ta9b6e9hkmxhtmcqvwd6ka8f";
...
app.use((req, res, next) => {
  ...
  const validSignature = chatClient.verifyWebhook(
    req.rawBody,
...
app.post("/", (req, res) => {
  ...
  // we are going to store the appointment in our database and update the message to show AddToCallendar component
    ...
    ).toISOString(); // add 30 minutes for the appointment duration
    message.mml = `
        <mml>
            <add_to_calendar
            title="${title}"
